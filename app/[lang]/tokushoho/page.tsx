...
export default async function TokushohoPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  
  const supportedLangs = ['ja', 'en', 'ko'];
  const currentLang = supportedLangs.includes(lang) ? lang : 'ja';

  const filePath = path.join(process.cwd(), 'docs', currentLang, 'tokushoho.md');
  let content = '';
  try {
        ...
        <div style={{
          ...
          {currentLang === 'ko' ? (
            <p style={{ margin: 0 }}>
              ...
              <Link href="/ja/tokushoho" style={{ color: '#2563eb', textDecoration: 'underline' }}>
                일본어판 보기 →
              </Link>
            </p>
          ) : (
